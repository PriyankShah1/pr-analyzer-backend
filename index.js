...
const { enrichWithTypes, detectMismatches } = require('./analyzer');
...
// ── 4. In-memory cache (public PRs only) ─────────────────────────────────
const cache = new Map();
...
function guardLargePR(phpFiles) {
  if (phpFiles.length > MAX_PHP_FILES) {
    throw new Error(
      `PR is too large: ${phpFiles.length} PHP files found. ` +
      `Max supported is ${MAX_PHP_FILES}.`
    );
  }
}
...
// ── 7. GitHub error handler — never expose token ──────────────────────────
...
// ── Global unhandled rejection — scrub token from logs ───────────────────
...
app.post('/analyze', rateLimit, async (req, res) => {
  ...
  // ── Per-request Octokit — user token takes priority ───────────────────
  const requestOctokit = new Octokit({
  ...
  try {
    const response = await requestOctokit.pulls.listFiles(repoInfo);
    let phpFiles = response.data.filter(file => file.filename.endsWith('.php'));
    ...
    if (phpFiles.length === 0) {
      return res.json({
        ...
        visualization: {
          nodes: [], edges: [],
          stats: { totalNodes: 0, totalEdges: 0, mismatches: 0, staticCalls: 0 },
        },
    ...
    let flows = parseLaravelFlow(phpFiles);
    flows = enrichWithTypes(phpFiles, flows);
    flows = detectMismatches(flows);

    const visualization = buildVisualizationResponse(flows);

    const result = {
      files: phpFiles.map(f => ({
        filename:  f.filename,
        truncated: f.truncated || false,
      })),
      flows,
      visualization,
      warnings: truncatedFiles.length > 0
        ? [`${truncatedFiles.length} file(s) were truncated (>${MAX_FILE_LINES} lines): ${truncatedFiles.join(', ')}`]
        : [],
    };

    // ── Cache result — only for public PRs (no user token) ────────────────
    if (!token) {
